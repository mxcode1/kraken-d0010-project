...
echo -e "${CYAN}🗄️ Setting up databases for Kraken D0010 ($DATABASE_CONFIG)...${NC}"
...
if [[ ! -d "venv" ]]; then
    echo -e "${RED}❌ Virtual environment not found. Please run setup_python_environment.sh first${NC}"
    exit 1
fi
...
setup_postgresql_database() {
    ...
    # Create database
    echo -e "${YELLOW}📊 Creating kraken_d0010 database...${NC}"
    createdb kraken_d0010 2>/dev/null || echo "Database may already exist"
    
    # Create user
    echo -e "${YELLOW}👤 Creating kraken_user...${NC}"
...
setup_sqlite_database() {
    echo -e "${YELLOW}📄 Setting up SQLite3 database...${NC}"
    
    # Remove existing database if present
    if [[ -f "db.sqlite3" ]]; then
        echo -e "${YELLOW}⚠️ Removing existing SQLite3 database...${NC}"
        rm -f db.sqlite3
    fi
    ...
    if python manage.py check --database=default &>/dev/null; then
        echo -e "${GREEN}✅ SQLite3 database ready${NC}"
    else
        echo -e "${RED}❌ SQLite3 setup failed${NC}"
        exit 1
    fi
}
...
setup_databases() {
    case $DATABASE_CONFIG in
        ...
        postgresql-primary)
            ...
            ;;
        *)
            echo -e "${RED}❌ Invalid database configuration: $DATABASE_CONFIG${NC}"
            exit 1
            ;;
    esac
}
...
display_status() {
    ...
    if [[ "$DATABASE_CONFIG" != "postgresql-primary" ]]; then
        ...
        local sqlite_check=$(python manage.py check --database=default 2>/dev/null && echo "✅ Ready" || echo "❌ Failed")
        echo -e "   Status: $sqlite_check"
        if [[ "$sqlite_check" == "✅ Ready" ]]; then
    ...
    if [[ "$DATABASE_CONFIG" == "both" || "$DATABASE_CONFIG" == "postgresql-primary" ]]; then
        ...
        local pg_check=$(USE_POSTGRESQL=1 python manage.py check --database=default 2>/dev/null && echo "✅ Ready" || echo "❌ Failed")
        echo -e "   Status: $pg_check"
        if [[ "$pg_check" == "✅ Ready" ]]; then
...
main() {
    ...
    if [[ "$DATABASE_CONFIG" == "both" ]]; then
        echo ""
        echo -e "${CYAN}💡 Dual Database Usage:${NC}"
